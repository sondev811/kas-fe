...
//function mới
export const sendQuotingMail = async (body: IQuotingForm) => {
  try {
    const { fullName } = body;
    let firstName = fullName && fullName.split(' ').slice(0, -1).join(' ');
    let lastName = fullName && fullName.split(' ').slice(-1).join(' ');
    const userBody = {...body, firstName, lastName};
    await axios.post("http://localhost:3001/users", userBody);
    await axios.post("http://localhost:3001/sendQuotingEmail", body);
  } catch (error) {
    throw error;
  }
}
